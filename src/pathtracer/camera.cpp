...
Ray Camera::generate_ray(double x, double y) const {
  ...
  // Note: hFov and vFov are in degrees.
    ...
    double x_screen = (2 * x - 1) * tan(hFov_rad / 2);
    double y_screen = (2 * y - 1) * tan(vFov_rad / 2);
	double z_screen = -1;

	Vector3D screen_pos = Vector3D(x_screen, y_screen, z_screen);

	// transform the canonical sensor plane position to world coordinates.
    Vector3D direction = c2w * screen_pos;
    Vector3D origin = pos;
    direction.normalize();

	// generate ray from camera position to the world position computed above.
